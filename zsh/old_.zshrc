...
precmd() { vcs_info, powerline_prompt }
...
function powerline_prompt() {
  local user_section="%F{$FG_BLUE}%n@%m%f %F{$BG_ALT}${RIGHT_ARROW}%f"
  local path_section="%F{$FG_CYAN}%~%f %F{$BG_ALT}${RIGHT_ARROW}%f"
  local git_section="%F{$FG_MAGENTA}${vcs_info_msg_0_}%f"
  local mode_section="$(vi_mode_prompt)"
  
  PROMPT="${user_section}${path_section}${git_section}${mode_section} ❯ "
}
